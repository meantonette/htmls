...
var shotsFired; // the number of shot the user has fired
...
var target; // start and endpoints of the target
...
var targetVelocity; // target spped multiplier duirng game
...
var cannonballVelocityX; // cannonball's velocity
var cannonballVelocityY; // cannonball's velocity
...
var barrelEnd; // the end point of the canon's barrel
...
function setupGame()
{
 ...
 document.addEventListener( "unload", stopTimer. false );
	...
	barrelEnd = new Object(); // object representing end ofcannon barrel
...
// called by function newGame to scale the size of the game elemnts
...
function resetElements()
{
	...
	// configure instance variable related to the blocker
	blockerDistance = w * 5 / 8; // blocker 5/8 canvas width from length
	...
	// configure instance variable related to the target
	...
	if (cannonballOnScreen) // if there is currently a shot fired
		{
			...
			if ( cannonballVelocityX > 0 && cannonball.x + cannonballRadius >= blockerDistance && cannonball.x + cannonballRadius <= blockerDistance + lineWidth && cannonball.y - cannonballRadius > blocker.start.y && cannonball.y + cannonballRadius < blocker.end.y)
				{
			...
			// check for cannonball collision with target
					...
					cannonball.y + cannonballRadius < target.end.y)
				{
					...
					// check wether the	piece  hasn't been hit yet
					if ((section >= 0 && section <  TARGET_PIECES) && !hitStates[section])
						{
							...
							timeLeft += HIT_REWARD; // add reward to the remaining time
...
} // end function alignCanon

// draw the game elementns to the given Canvas
function draw()
{
